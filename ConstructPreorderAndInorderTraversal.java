...
public class ConstructPreorderAndInorderTraversal {
	...
	public static TreeNodeI buildTree(int[] preorder, int[] inorder) {
        ...
        TreeNodeI root = build(inorderTable,preorder,0,preorder.length-1,inorder,0,inorder.length-1);
		return root;
    }
	
	public static TreeNodeI build(HashMap<Integer,Integer> inorderTable, int[] preorder,int pcur, int pend,int[] inorder, int istart, int iend){
		...
		root.left = build(inorderTable,preorder,pcur+1,pcur+1+len-1,inorder,istart,end-1);  //what is pcur here
		root.right = build(inorderTable,preorder,pcur+1+len-1+1,pend,inorder,end+1,iend);
